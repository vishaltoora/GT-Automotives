...
if ($result->num_rows > 0) {
    ...
    $update_query = "UPDATE users SET password = ?, email = ?, is_admin = ? WHERE username = ?";
    $update_stmt = $conn->prepare($update_query);
    $update_stmt->bind_param("ssis", $hashed_password, $email, $is_admin, $username);
    
    if ($update_stmt->execute()) {
        echo "User 'rohit.toora' password updated successfully!\n";
...
$verify_query = "SELECT id, username, email, is_admin FROM users WHERE username = ?";
