...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO locations (name, address, phone, email, hours, is_active) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssi", $name, $address, $phone, $email, $hours, $is_active);
    ...
    <form action="" method="POST">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <textarea name="address" id="address" rows="2" required><?php echo htmlspecialchars($_POST['address'] ?? ''); ?></textarea>
            </div>
        </div>
