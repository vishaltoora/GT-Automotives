...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $location_id = intval($_POST['location_id'] ?? 1); // Default to first location
    ...
    $uploaded_photos = [];
    if ($condition === 'used' && isset($_FILES['photos']) && !empty($_FILES['photos']['name'][0])) {
        ...
        foreach ($_FILES['photos']['tmp_name'] as $index => $tmp_name) {
            if ($_FILES['photos']['error'][$index] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($tmp_name, $upload_path)) {
                    $uploaded_photos[] = 'images/used_tires/photos/' . $unique_name;
                } else {
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <select name="product_type" id="product_type" class="select2-enhanced" required>
                    ...
                    <option value="Winter Tires" <?php echo (isset($_POST['product_type']) && $_POST['product_type'] === 'Winter Tires') ? 'selected' : ''; ?>>Winter Tires</option>
                    <option value="Summer Tires" <?php echo (isset($_POST['product_type']) && $_POST['product_type'] === 'Summer Tires') ? 'selected' : ''; ?>>Summer Tires</option>
                    <option value="All Season Tires" <?php echo (isset($_POST['product_type']) && $_POST['product_type'] === 'All Season Tires') ? 'selected' : ''; ?>>All Season Tires</option>
                    <option value="Studded Tires" <?php echo (isset($_POST['product_type']) && $_POST['product_type'] === 'Studded Tires') ? 'selected' : ''; ?>>Studded Tires</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="size" id="size" class="select2-enhanced" required>
                    ...
                    <?php foreach ($sizes as $size): ?>
                        <option value="<?php echo htmlspecialchars($size['name']); ?>" <?php echo (isset($_POST['size']) && $_POST['size'] === $size['name']) ? 'selected' : ''; ?>>
            ...
            <div class="form-group">
                ...
                <input type="number" name="price" id="price" min="0.01" step="0.01" value="<?php echo htmlspecialchars($_POST['price'] ?? ''); ?>" required>
            </div>
            
            <div class="form-group">
                <label for="stock_quantity">Stock Quantity</label>
                <input type="number" name="stock_quantity" id="stock_quantity" min="0" value="<?php echo htmlspecialchars($_POST['stock_quantity'] ?? '0'); ?>" required>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="description" id="description" rows="5"><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
        </div>
        
        <!-- Used Tire Photos Section -->
        <div id="used-tire-photos" style="display: none;">
            <div class="form-group">
                <label for="photos">Upload Photos (for used tires)</label>
                <input type="file" name="photos[]" id="photos" multiple accept="image/*" class="form-control">
                <small class="form-text text-muted">
                    Upload multiple photos of the used tire. Allowed formats: JPG, PNG, GIF. Maximum file size: 5MB per image.
                </small>
            </div>
